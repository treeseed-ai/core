...
export function resolveSharedFixtureRoot() {
	...
	for (const entry of readdirSync(sitesRoot, { withFileTypes: true })) {
		...
		try {
			...
			if (existsSync(join(root, 'src', 'manifest.yaml'))) {
				return root;
			}
		} catch {
			continue;
...
export function checkSharedFixture() {
	...
	if (!existsSync(join(fixtureRoot, 'src', 'manifest.yaml'))) {
		throw new Error(`Shared fixture is missing src/manifest.yaml at ${fixtureRoot}.`);
	}
