...
export function detectBootstrapMode(startRoot = process.cwd()) {
...
export function runWorkspaceBootstrap({ root = process.cwd() } = {}) {
	const resolvedRoot = resolve(root);
	const state = detectBootstrapMode(resolvedRoot);
...
if (isDirectEntrypoint()) {
	try {
		process.exitCode = runWorkspaceBootstrap();
	} catch (error) {
