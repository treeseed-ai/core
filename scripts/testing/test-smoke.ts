...
function resolveWorkspaceRuntimePackageRoots() {
	...
	for (const dependencyName of runtimeDependencyNamesFor(packageRoot)) {
		const dependencyRoot = resolveRuntimeDependencyRoot(dependencyName);
		if (dependencyRoot) {
...
function collectRuntimeDependenciesForPackaging() {
	...
	while (queue.length > 0) {
		...
		for (const dependencyName of runtimeDependencyNamesFor(nextRoot)) {
			...
			const dependencyRoot = resolveRuntimeDependencyRoot(dependencyName, searchRoots);
			if (dependencyRoot) {
...
function resolveRuntimeDependencyRoot(packageName: string, searchRoots: string[] = [packageRoot, sdkPackageRoot]) {
...
function pack(root: string, outputRoot: string, fallbackName: string) {
	...
	const hasSourceBuild = existsSync(resolve(root, 'scripts', 'build-dist.ts'))
		|| existsSync(resolve(root, 'scripts', 'build', 'build-dist.ts'));
	if (bundledTarball && !hasSourceBuild) {
