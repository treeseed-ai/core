...
- build(build): checkpoint release recovery fixes (5c81fbdf1eb1)
...
- build(build): prevent stale stage auto-resume after candidate heads (a168716860c6)
- build(build): allow retained production aliases during staging (164932286515)
- build(build): deduplicate Railway API services and enforce canonical (9f9b1f07e5a9)

## [0.12.56] - 2026-07-13

### Fixed

- build(build): fix release image state and atomic lockfiles (a56cd9359d8b)

## [0.12.55] - 2026-07-13

### Fixed

- build(build): scope release validation to selected packages (8c1e081ab867)
- build(build): fix selective release metadata for verification-only (c34320523285)
...
- build(build): discover Railway sibling identities without production (d602007781f8)
- build(build): retain sibling Railway environments during production (744f5ec6aef8)
- build(build): retain sibling Railway environments during production (18505134ff6b)

## [0.12.51] - 2026-07-13

### Fixed

- build(build): Fix release readiness and merged branch cleanup (d14c6e9ad5b9)

### Dependencies

- build(build): Retry transient GitHub job setup failures once (8476b78c9bdd)
- build(build): Avoid repeated fetches during merged branch cleanup (775b88ec9b11)
- build(build): Delete merged feature branches after successful stage (c8d462086e3c)
- build(build): Require explicit workflow resume and prevent stale (7e0a3cd95e5b)
- build(build): Permanently isolate Railway staging Git sources (54230789766a)

## [0.12.50] - 2026-07-13

### Dependencies

- build(deps): update @treeseed/sdk and bump version (a9187433bf3d)

## [0.12.49] - 2026-07-12

### Dependencies

- build(core): update version and @treeseed/sdk dependency (2728307deef3)

## [0.12.48] - 2026-07-12

### Dependencies

- chore(deps): update @treeseed/sdk to github commit (c15ddd7439d3)

## [0.12.47] - 2026-07-12

### Dependencies

- build(core): update @treeseed/sdk dependency and package version (c07c09831e44)

## [0.12.46] - 2026-07-12

### Dependencies

- build(core): update @treeseed/sdk dependency and package version (e8136b12c341)

## [0.12.45] - 2026-07-12

### Dependencies

- build(build): preserve registry dependency lock semantics (d3393785fe5f)

## [0.12.44] - 2026-07-12

### Dependencies

- build(build): reuse exact staged candidate proof during release (1b8c20d7b8a7)
- build(build): reuse staged package closure during release verification (d67d4e9809be)
- build(build): resolve managed release tooling and stale checkout (472816e4ff0d)
- build(build): resolve managed release tooling and stale checkout (cf7f05459d1a)

## [0.12.43] - 2026-07-12

### Tests

- build(build): verify local-only reviewer through workspace links (412ed4ca6f22)

### Dependencies

- build(build): allow Railway deployments to settle before live (0756ee121946)
- build(build): restore workspace links after release dependency hydration (f100b4169715)
- build(build): restore workspace links after release dependency hydration (b26190904b0a)
- build(build): hydrate missing package dependencies before release (53fb9b4cdedf)
- build(build): avoid release tag collisions after partial publication (f863f1382295)
- chore(deps): update @treeseed/sdk and bump version (c5490f2e777c)
- build(deps): update @treeseed/sdk dependency (8d4f6f07788b)
- chore(deps): update @treeseed/sdk dependency pointer (b5b061ef488f)
- build(core): bump version and update dependencies (4cb4a8bae85c)
- build(core): update @treeseed/sdk dependency pointer (a7e87e768fa8)
- build(core): update @treeseed/sdk dependency pointer (901bf535d775)
- build(core): bump version and update dependencies (5600aef506d1)

## [0.12.42] - 2026-07-11

### Fixed

- build(build): fix stage completion and API SMTP routing (81945397f031)
- build(build): Bind hosted API SMTP settings from root staging registry (2b342c77b895)
- build(build): fix staging candidate credential handoff (0aa71416e25b)
- build(build): lock in UI shell architecture and test fixes (84115e40e4be)

### Tests

- build(build): Standardize verify release-gate and deploy workflows (2cfafa7f4dc7)
- ci(ci): recover reliable save stage release workflow (20e00f32912d)

### Dependencies

- build(build): Exclude verification-only packages from release (2d4c4a7c6e7b)
- build(build): seed verified login fixture without browser session (3a97ba81c8dc)
- build(build): preserve dependency graph during artifact hydration (634c4ef5b2cd)
- build(build): hydrate exact candidate artifacts before staging deploy (516e1447e215)
- build(build): Propagate API web service secret through staging (92aa102c2ff3)
- build(build): Propagate control-plane secrets through staging (5ed7d26bd35d)
- build(build): Expose API surface URLs to strict live staging checks (9b8061480695)
- build(build): Make hosted apply wait for transient HTTP readiness (a4d65f736952)
- build(build): inject protected TreeDX secrets into staging (da4742115394)
- build(build): make staging candidate retries resumable (baa9fc8ec08d)
- build(build): release guarantee reviewer and coverage updates (bd2990fc6f3b)
- build(build): release guarantee reviewer and coverage updates (97cf205beb73)
- build(build): release guarantee reviewer and coverage updates (43478415ef65)
- build(build): release guarantee reviewer and coverage updates (4910017f5e3a)
- build(build): guard API workflow entrypoints and staging fanout (546b0053b662)
- build(build): block dependents on API deploy workflow (a17e226a9a7d)
- chore(deps): update @treeseed/sdk dependency pointer (0d1e7a9185a0)
...
- build(build): add final production release guarantee gate (4ed92a6d673d)

## [0.12.36] - 2026-07-05

### Fixed

- build(build): fix production source cache purge finalization (01165ac684c9)

## [0.12.35] - 2026-07-04

### Dependencies

- build(build): bypass source page edge cache for production release (0d71e40b7a91)
- build(build): make live hosted env checks provider authoritative (4ae08b384ad8)
- build(build): fail release on broken production web surface (8a154378454b)

## [0.12.34] - 2026-07-04

### Dependencies

- build(build): purge production web cache before release verification (89c418a388ed)
- build(build): purge production web cache before release verification (7b2f9b95b5fc)
...
- docs: clean release changelog (801a87d2d449)
