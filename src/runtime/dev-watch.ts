...
export type DevWatchEntry = {
	kind: 'tenant' | 'core' | 'sdk' | 'agent' | 'cli';
	root: string;
	restartRequired?: boolean;
};

export type DevWatchChange = {
	changedPaths: string[];
...
export type DevWatchController = {
	stop: () => void;
	rebaseline: () => void;
};

export type DevWatchStarter = (
	input: {
		watchEntries: DevWatchEntry[];
		onChange: (change: DevWatchChange) => void | Promise<void>;
	},
) => DevWatchController;
...
function collectSnapshot(entries: DevWatchEntry[]) {
...
export function classifyChanges(changedPaths: string[], watchEntries: DevWatchEntry[]): DevWatchChange {
	function matchesEntry(filePath: string, entry: DevWatchEntry) {
...
export function startPollingWatch({ watchEntries, onChange }: Parameters<DevWatchStarter>[0]) {
