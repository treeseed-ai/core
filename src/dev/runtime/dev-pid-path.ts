...
import { DEFAULT_API_PORT, DEFAULT_MARKET_POSTGRES_PORT, DEFAULT_WEB_PORT, type ProcessStatusChecker, type DevIndex, type DevIndexEntry, type DevInstanceRecord, type DevInstanceStatus, type IntegratedDevPlan, type ManagedDevOptions } from '../configuration/runtime-configuration.ts';
...
export function portsFromPlan(plan: IntegratedDevPlan) {
...
export function urlsFromPlan(plan: IntegratedDevPlan) {
	return Object.fromEntries(
		Object.entries({
			web: plan.webUrl,
			api: plan.apiBaseUrl,
...
export function readDevInstanceFile(path: string): DevInstanceRecord | null {
	try {
		const parsed = JSON.parse(readFileSync(path, 'utf8')) as Partial<DevInstanceRecord>;
		if (parsed.kind !== 'treeseed.dev.instance' || typeof parsed.projectRoot !== 'string' || typeof parsed.instancePath !== 'string') {
			return null;
		}
		return parsed as DevInstanceRecord;
	} catch {
		return null;
	}
}

export function writeDevInstance(record: DevInstanceRecord) {
...
export function readRepoFamilyIndex(tenantRoot: string, gitCommonDir: string | null): DevIndex {
	const path = repoFamilyIndexPath(tenantRoot, gitCommonDir);
	try {
		const parsed = JSON.parse(readFileSync(path, 'utf8')) as Partial<DevIndex>;
		if (parsed.kind === 'treeseed.dev.index' && Array.isArray(parsed.instances)) {
			return parsed as DevIndex;
		}
	} catch {
...
export function writeRepoFamilyIndex(index: DevIndex, tenantRoot: string, gitCommonDir: string | null) {
	atomicWriteJson(repoFamilyIndexPath(tenantRoot, gitCommonDir), index);
}

export function writeRepoFamilyIndexEntry(record: DevInstanceRecord) {
	const index = readRepoFamilyIndex(record.projectRoot, record.gitCommonDir);
	const entry: DevIndexEntry = {
...
export function removeRepoFamilyIndexEntry(record: DevInstanceRecord) {
	const index = readRepoFamilyIndex(record.projectRoot, record.gitCommonDir);
...
export function listWorktreeDevInstances(tenantRoot: string) {
	try {
		return readdirSync(devInstanceDir(tenantRoot))
			...
			.filter((entry): entry is DevInstanceRecord => Boolean(entry));
	} catch {
		return [];
	}
}
...
export function evaluateDevInstance(
	...
	deps: Pick<IntegratedDevDependencies, 'processIsAlive'>,
): DevInstanceRecord {
...
export function removeDevInstanceRecord(record: DevInstanceRecord) {
	rmSync(record.instancePath, { force: true });
	rmSync(record.pidPath, { force: true });
	removeRepoFamilyIndexEntry(record);
}

export function usedPortsFromInstances(records: readonly DevInstanceRecord[], processIsAlive: ProcessStatusChecker) {
...
export function managedPortBlock(blockIndex: number) {
	...
	return {
		web: DEFAULT_WEB_PORT + offset,
		api: DEFAULT_API_PORT + offset,
		postgres: DEFAULT_MARKET_POSTGRES_PORT + offset,
	};
}
...
export function resolveManagedPortOverrides(
	...
	deps: Pick<IntegratedDevDependencies, 'processIsAlive' | 'inspectPortOwners'>,
) {
...
export function renderManagedDevStatus(records: readonly DevInstanceRecord[]) {
