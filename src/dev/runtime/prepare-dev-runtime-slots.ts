...
import type { IntegratedDevOptions, IntegratedDevPlan, IntegratedDevResetAction, IntegratedDevResetPlan } from '../configuration/runtime-configuration.ts';
import type { DevEvent, IntegratedDevDependencies } from './integrated-dev-dependencies.ts';
...
import { resetMarketPostgres } from '../support/attach-prefixed-log-reader.ts';
...
export async function prepareDevRuntimeSlots(
	...
	options: Pick<IntegratedDevOptions, 'json' | 'shutdownGraceMs' | 'force'>,
	deps: Pick<IntegratedDevDependencies, 'write' | 'killProcess' | 'processIsAlive' | 'inspectPortOwners'>,
) {
...
export function emitEvent(
	options: Pick<IntegratedDevOptions, 'json'>,
	write: IntegratedDevDependencies['write'],
	event: DevEvent,
...
export function createDevLogWrite(
	baseWrite: IntegratedDevDependencies['write'],
	logPath: string,
): IntegratedDevDependencies['write'] {
...
export function runIntegratedDevReset(
	reset: IntegratedDevResetPlan | null,
	options: Pick<IntegratedDevOptions, 'json'>,
	deps: Pick<IntegratedDevDependencies, 'write' | 'removePath' | 'stopMailpitContainers' | 'resetMarketPostgres'>,
) {
	if (!reset?.enabled) {
		return null;
	}
	const results = reset.actions.map((action) => {
		...
		if (action.kind === 'service') {
			...
			const result: IntegratedDevResetAction = {
				...action,
...
export function writePlan(plan: IntegratedDevPlan, options: Pick<IntegratedDevOptions, 'json'>, write: IntegratedDevDependencies['write']) {
	if (options.json) {
