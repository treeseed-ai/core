...
import { DEFAULT_KILL_GRACE_MS, DEFAULT_SHUTDOWN_GRACE_MS, DEV_RUNTIME_DIR, DEV_RUNTIME_LEGACY_FILE, type ProcessStatusChecker, type DevInstanceStatus, type IntegratedDevCommandId, type IntegratedDevOptions, type IntegratedDevPlan } from '../configuration/runtime-configuration.ts';
import { ALL_COMMAND_IDS, CANONICAL_COMMAND_IDS, type DevPortOwner, type IntegratedDevDependencies } from './integrated-dev-dependencies.ts';
...
export type DevRuntimeState = {
	...
	tenantRoot: string;
	startedAt: string;
	commandIds?: IntegratedDevCommandId[];
	statePath?: string;
};
...
export function runtimeScopeKey(commandIds: readonly IntegratedDevCommandId[]) {
...
export function readDevRuntimeStateFile(path: string): DevRuntimeState | null {
	try {
		...
		const commandIds = Array.isArray(parsed.commandIds)
			? parsed.commandIds.filter((id): id is IntegratedDevCommandId => ALL_COMMAND_IDS.includes(id as IntegratedDevCommandId))
			: undefined;
		return {
			pid: parsed.pid!,
...
export function runtimeStateOverlaps(state: DevRuntimeState, commandIds: readonly IntegratedDevCommandId[]) {
...
export function listLiveOverlappingDevRuntimeStates(
	tenantRoot: string,
	commandIds: readonly IntegratedDevCommandId[],
...
export function requiredDevPorts(plan: IntegratedDevPlan) {
	const ports: number[] = [];
...
export function formatPortOwner(owner: DevPortOwner) {
	return `port ${owner.port}${owner.pid ? ` pid ${owner.pid}` : ''}${owner.processName ? ` (${owner.processName})` : ''}`;
}

export function writeCurrentDevRuntimeState(plan: IntegratedDevPlan, status: DevInstanceStatus = 'starting') {
...
export function updateCurrentDevRuntimeState(plan: IntegratedDevPlan, status: DevInstanceStatus, staleReason?: string) {
...
export function removeCurrentDevRuntimeState(plan: IntegratedDevPlan) {
	const tenantRoot = plan.tenantRoot;
	const commandIds = plan.commands.map((command) => command.id);
...
export async function stopPreviousDevRuntimes(
	tenantRoot: string,
	commandIds: readonly IntegratedDevCommandId[],
	options: Pick<IntegratedDevOptions, 'json' | 'shutdownGraceMs'>,
	deps: Pick<IntegratedDevDependencies, 'write' | 'killProcess' | 'processIsAlive'>,
) {
