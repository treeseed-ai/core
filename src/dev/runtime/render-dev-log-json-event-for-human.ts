...
import { DEFAULT_KILL_GRACE_MS, DEFAULT_READINESS_TIMEOUT_MS, DEFAULT_SHUTDOWN_GRACE_MS, type DevInstanceRecord, type ManagedDevOptions } from '../configuration/runtime-configuration.ts';
import type { ManagedStartDependencies, IntegratedDevDependencies } from './integrated-dev-dependencies.ts';
...
import { createIntegratedDevPlan } from '../support/create-api-command.ts';
...
export async function waitForManagedInstanceReady(
	instancePath: string,
	options: Pick<ManagedDevOptions, 'readinessTimeoutMs'>,
...
export function managedDevResult(kind: string, ok: boolean, payload: unknown, options: Pick<ManagedDevOptions, 'json'>, write: IntegratedDevDependencies['write']) {
	if (options.json) {
		write(`${JSON.stringify({ schemaVersion: 1, kind, ok, payload }, null, 2)}\n`, 'stdout');
	} else if (typeof payload === 'string') {
		write(`${payload}\n`, 'stdout');
	} else if (Array.isArray(payload)) {
		write(`${renderManagedDevStatus(payload as DevInstanceRecord[])}\n`, 'stdout');
	} else {
		write(`${renderManagedDevStatus([payload as DevInstanceRecord])}\n`, 'stdout');
	}
	return ok ? 0 : 1;
...
export async function stopDevInstance(
	...
	options: Pick<ManagedDevOptions, 'shutdownGraceMs'>,
	deps: Pick<IntegratedDevDependencies, 'killProcess' | 'processIsAlive'>,
) {
...
export async function runManagedDev(
	...
	if (options.action === 'restart') {
		await runManagedDev({ ...options, action: 'stop' }, { ...deps, write: () => {} });
		return runManagedDev({ ...options, action: 'start' }, deps);
	}
