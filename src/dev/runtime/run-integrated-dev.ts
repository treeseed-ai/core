...
import { startPollingWatch } from "../../runtime/dev-watch";
import { DEFAULT_PROCESS_READY_GRACE_MS, DEFAULT_READINESS_TIMEOUT_MS, DEFAULT_SHUTDOWN_GRACE_MS, INITIAL_RESTART_BACKOFF_MS, MAX_RESTART_BACKOFF_MS, SETUP_RETRY_BACKOFF_MS, type IntegratedDevCommand, type IntegratedDevOptions, type WatchController } from '../configuration/runtime-configuration.ts';
...
import { createIntegratedDevPlan, defaultPrepareEnvironment, defaultSignalRegistrar } from '../support/create-api-command.ts';
import { defaultOpenBrowser, failedSetupMessage, runLocalSetup, shouldOpenBrowser, waitForHttpReady } from './run-local-setup.ts';
import { attachPrefixedLogReader, resetMarketPostgres, stopMarketPostgres } from '../support/attach-prefixed-log-reader.ts';
import { createDevLogWrite, emitEvent, prepareDevRuntimeSlots, runIntegratedDevReset, writePlan } from './prepare-dev-runtime-slots.ts';
import { removeCurrentDevRuntimeState, updateCurrentDevRuntimeState, writeCurrentDevRuntimeState } from './dev-runtime-state.ts';

export async function runIntegratedDev(
	options: IntegratedDevOptions = {},
	deps: Partial<IntegratedDevDependencies> = {},
) {
	...
	const resetResults = runIntegratedDevReset(plan.reset, options, {
		write,
		removePath,
	...
	const children = new Map<IntegratedDevCommand['id'], ManagedDevProcess>();
	...
	const restartAttempts = new Map<IntegratedDevCommand['id'], number>();
	const restartTimers = new Map<IntegratedDevCommand['id'], NodeJS.Timeout>();
	...
	return await new Promise<number>((resolveExitCode) => {
		...
		function spawnCommand(command: IntegratedDevCommand) {
			emitEvent(options, write, {
				type: 'spawn',
				surface: command.id,
		...
		function scheduleCommandRestart(id: IntegratedDevCommand['id']) {
		...
		async function restartCommand(id: IntegratedDevCommand['id']) {
			const command = commandsById.get(id);
			if (!command || settled) {
				return;
			}
			...
			watchController = startWatch({
				...
				onChange: async (change) => {
					...
					try {
						...
						const restartIds = new Set<IntegratedDevCommand['id']>();
		...
		async function waitForReadiness() {
			...
			for (const check of plan.readyChecks) {
				...
				if (check.strategy === 'http' && check.url) {
					...
					const commandId = check.id as IntegratedDevCommand['id'];
