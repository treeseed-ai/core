...
export function defaultInspectPortOwners(ports: readonly number[]): DevPortOwner[] {
...
export function createManagedDevProcess(command: IntegratedDevCommand, child: ProcessLike): ManagedDevProcess {
...
export function serializeDevPlanForOutput(plan: IntegratedDevPlan): IntegratedDevPlan {
	return {
		...plan,
...
export function resolveLocalMachineEnv(tenantRoot: string) {
	try {
		return resolveMachineEnvironmentValues(tenantRoot, 'local') as NodeJS.ProcessEnv;
	} catch {
		return {};
	}
}
...
export function runGitText(cwd: string, args: string[]) {
	const result = runRepositoryGit(args, { cwd, mode: 'read', allowFailure: true });
	return (result.status ?? 1) === 0 ? String(result.stdout ?? '').trim() : null;
}

export function isInsideInternalPath(worktreeRoot: string, tenantRoot: string) {
...
export function resolveGitWorktreeInfo(tenantRoot: string) {
	...
	const isTemporaryRoot = resolvedWorktreeRoot
		? relative(resolve(resolvedWorktreeRoot, '.treeseed', 'tmp'), resolve(tenantRoot)).split(sep)[0] !== '..'
		: false;
	if (!resolvedWorktreeRoot || isInsideInternalPath(resolvedWorktreeRoot, tenantRoot) || isTemporaryRoot) {
...
export function instanceRuntimeScope(plan: Pick<IntegratedDevPlan, 'commands'>) {
