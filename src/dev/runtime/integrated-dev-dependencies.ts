...
export type IntegratedDevDependencies = {
	spawn: SpawnLike;
	...
	inspectPortOwners: (ports: readonly number[]) => DevPortOwner[];
};

export type ManagedStartDependencies = Pick<IntegratedDevDependencies, 'spawn' | 'write' | 'fetch' | 'processIsAlive' | 'killProcess' | 'inspectPortOwners'>;

export type DevPortOwner = {
	port: number;
	pid: number | null;
	processName?: string;
	detail: string;
...
export function normalizeSetupMode(value: IntegratedDevSetupMode | undefined) {
	return value ?? 'auto';
}

export function normalizeFeedbackMode(value: IntegratedDevFeedbackMode | undefined) {
	return value ?? 'live';
}

export function normalizeOpenMode(value: IntegratedDevOpenMode | undefined) {
	return value ?? 'off';
}
...
export function selectWebLocalRuntime(
	...
	overrideRuntime?: LocalRuntimeMode,
...
export const CANONICAL_COMMAND_IDS: IntegratedDevCommandId[] = ['web', 'api', 'manager', 'worker', 'agents'];

export const ALL_COMMAND_IDS: IntegratedDevCommandId[] = ['web', 'api', 'manager', 'worker', 'agents', 'operations-runner'];

export const MARKET_DEV_COMMAND_IDS: IntegratedDevCommandId[] = ['web', 'api', 'operations-runner'];
...
export function surfaceCommandIds(surface: IntegratedDevSurface): IntegratedDevCommandId[] {
	switch (surface) {
		case 'web':
			return ['web'];
		case 'api':
