...
import { ensureMarketPostgres, runSetupStep } from '../support/attach-prefixed-log-reader.ts';
...
export function runLocalSetup(
	...
	deps: Pick<IntegratedDevDependencies, 'spawnSync' | 'write'>,
) {
	const results: IntegratedDevSetupStep[] = [];
	...
	for (const step of plan.setupSteps) {
		...
		if (step.id === 'workspace-links') {
			if (plan.setupMode === 'check') {
				...
				const workspaceRoot = findNearestWorkspaceRoot(plan.tenantRoot);
				if (workspaceRoot) {
			...
			const wrangler = resolveToolBinary('wrangler', { env: { ...process.env, ...plan.commands[0]?.env } });
			result = wrangler
...
export function shouldOpenBrowser(plan: IntegratedDevPlan) {
...
export function failedSetupMessage(failed: IntegratedDevSetupStep) {
	return [
		`${failed.label} failed.`,
