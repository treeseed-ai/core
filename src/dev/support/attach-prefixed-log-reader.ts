...
export function runSetupStep(
	step: IntegratedDevSetupStep,
	plan: IntegratedDevPlan,
	deps: Pick<IntegratedDevDependencies, 'spawnSync'>,
) {
	if (!step.command || !step.args) {
		return {
			...step,
			status: step.status === 'failed' ? 'failed' : 'skipped',
		} satisfies IntegratedDevSetupStep;
	}
...
export function marketPostgresConfig(env: NodeJS.ProcessEnv) {
	return {
		container: env.TREESEED_MARKET_LOCAL_POSTGRES_CONTAINER?.trim() || DEFAULT_MARKET_POSTGRES_CONTAINER,
		volume: env.TREESEED_MARKET_LOCAL_POSTGRES_VOLUME?.trim() || DEFAULT_MARKET_POSTGRES_VOLUME,
		port: env.TREESEED_MARKET_LOCAL_POSTGRES_PORT?.trim() || String(DEFAULT_MARKET_POSTGRES_PORT),
		user: 'treeseed',
		password: 'treeseed',
...
export function dockerBinary(env: NodeJS.ProcessEnv) {
	return resolveToolBinary('docker', { env }) ?? 'docker';
}
...
export function resetMarketPostgres(env: NodeJS.ProcessEnv, deps: Pick<IntegratedDevDependencies, 'spawnSync'>) {
	const config = marketPostgresConfig(env);
...
export function stopMarketPostgres(env: NodeJS.ProcessEnv, deps: Pick<IntegratedDevDependencies, 'spawnSync'>) {
