...
export function createWatchEntries(tenantRoot: string, roots: {
...
}): IntegratedDevWatchEntry[] {
	const entries: IntegratedDevWatchEntry[] = [
	...
	try {
		for (const app of discoverApplications(tenantRoot)) {
...
export function createSetupSteps(
	...
	sdkPackageRoot: string,
	planLike: Pick<IntegratedDevPlan, 'commands'>,
	env: NodeJS.ProcessEnv,
	usesCloudflareWebRuntime: boolean,
): IntegratedDevSetupStep[] {
	if (setupMode === 'off') {
		return [
			{
				id: 'setup-disabled',
	...
	const marketMigrateScript = existsSync(resolve(apiPackageRoot, 'scripts/migrate-db.ts'))
		...
		: null;
	const steps: IntegratedDevSetupStep[] = [
		{
			id: 'workspace-links',
		...
		] : []),
		{
			...
			status: 'planned',
			detail: resolveToolBinary('wrangler', { env }) ?? undefined,
		},
...
export function createAgentCommand(
	id: Extract<IntegratedDevCommandId, 'api' | 'manager' | 'worker' | 'agents'>,
	tenantRoot: string,
	...
	const configs: Record<Extract<IntegratedDevCommandId, 'api' | 'manager' | 'worker' | 'agents'>, {
		label: string;
		source: string;
		dist: string;
		extraArgs?: string[];
