...
import { applyEnvironmentToProcess, assertCommandEnvironment, resolveWranglerBin } from '../../runtime/platform/workflow-support.ts';
import { INITIAL_RESTART_BACKOFF_MS, MAX_RESTART_BACKOFF_MS, SETUP_RETRY_BACKOFF_MS, DEFAULT_API_HOST, DEFAULT_API_PORT, DEFAULT_LOCAL_SMTP_HOST, DEFAULT_LOCAL_SMTP_PORT, DEFAULT_MAILPIT_UI_PORT, DEFAULT_MARKET_POSTGRES_CONTAINER, DEFAULT_MARKET_POSTGRES_PORT, DEFAULT_MARKET_POSTGRES_VOLUME, DEFAULT_WEB_HOST, DEFAULT_WEB_PORT, type IntegratedDevCommand, type IntegratedDevCommandId, type IntegratedDevOptions, type IntegratedDevPlan, type IntegratedDevReadinessCheck } from '../configuration/runtime-configuration.ts';
import { createIntegratedDevResetPlan, isManagedMarketPostgresUrl, nodeLocalRuntime, resolveSeededLocalProjectId, resolveSeededLocalTeamId, selectedSurfaceCommandIds } from './parse-surface-value.ts';
import { MARKET_DEV_COMMAND_IDS, fallbackWebProviderFromDeployConfig, generatedLocalWranglerPath, isMarketWorkspace, loadDevDeployConfig, normalizeFeedbackMode, normalizeOpenMode, normalizePort, normalizeSetupMode, resolveNodeEntrypoint, resolveOptionalPackageRoot, resolvePackageRoot, resolvePackageRootEnvOverride, selectWebLocalRuntime, webUrlFor } from '../runtime/integrated-dev-dependencies.ts';
import { resolveLocalMachineEnv } from '../runtime/default-kill-process.ts';
import { createAgentCommand, createSetupSteps, createWatchEntries } from './create-watch-entries.ts';
import { runtimeScopeKey } from '../runtime/dev-runtime-state.ts';
...
export function createIntegratedDevPlan(options: IntegratedDevOptions = {}): IntegratedDevPlan {
	...
	const webHost = options.webHost ?? DEFAULT_WEB_HOST;
	const webPort = normalizePort(options.webPort, DEFAULT_WEB_PORT);
	const apiHost = options.apiHost ?? DEFAULT_API_HOST;
	const apiPort = normalizePort(options.apiPort, DEFAULT_API_PORT);
	...
	const marketPostgresPort = mergedEnv.TREESEED_MARKET_LOCAL_POSTGRES_PORT ?? String(DEFAULT_MARKET_POSTGRES_PORT);
	const apiDatabaseUrl = mergedEnv.TREESEED_DATABASE_URL
		?? `postgres://treeseed:treeseed@127.0.0.1:${marketPostgresPort}/market_local`;
	const managedMarketPostgres = marketWorkspace && isManagedMarketPostgresUrl(apiDatabaseUrl);
	const mailpitSmtpPort = mergedEnv.TREESEED_MAILPIT_SMTP_PORT ?? String(DEFAULT_LOCAL_SMTP_PORT);
	...
	const sharedEnv: NodeJS.ProcessEnv = {
		...mergedEnv,
		LOCAL_DEV_MODE: usesCloudflareWebRuntime ? (mergedEnv.LOCAL_DEV_MODE ?? 'cloudflare') : undefined,
		...
		...(marketWorkspace ? {
			...
			TREESEED_MARKET_LOCAL_POSTGRES_CONTAINER: mergedEnv.TREESEED_MARKET_LOCAL_POSTGRES_CONTAINER ?? DEFAULT_MARKET_POSTGRES_CONTAINER,
			TREESEED_MARKET_LOCAL_POSTGRES_VOLUME: mergedEnv.TREESEED_MARKET_LOCAL_POSTGRES_VOLUME ?? DEFAULT_MARKET_POSTGRES_VOLUME,
		...
		FORM_TOKEN_SECRET: mergedEnv.FORM_TOKEN_SECRET ?? 'treeseed-local-form-token-secret',
		...
		SMTP_HOST: DEFAULT_LOCAL_SMTP_HOST,
		SMTP_PORT: mailpitSmtpPort,
		SMTP_USERNAME: '',
		SMTP_PASSWORD: '',
		TREESEED_MAILPIT_SMTP_HOST: DEFAULT_LOCAL_SMTP_HOST,
	...
	const reset = createIntegratedDevResetPlan({
		tenantRoot,
		env: sharedEnv,
	...
	const commands: IntegratedDevCommand[] = [];
	...
	for (const id of effectiveCommandIds) {
		...
		commands.push(createAgentCommand(id as Extract<IntegratedDevCommandId, 'api' | 'manager' | 'worker' | 'agents'>, tenantRoot, agentPackageRoot!, sharedEnv, apiHost, apiPort));
	}

	const readyChecks: IntegratedDevReadinessCheck[] = commands.map((command) => {
