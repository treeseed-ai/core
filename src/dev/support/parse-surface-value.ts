...
export function parseSurfaceValue(value: string): IntegratedDevSurface | null {
	return (
		value === 'web' ||
		value === 'api' ||
		value === 'manager' ||
...
export function selectedSurfaceCommandIds(options: Pick<IntegratedDevOptions, 'surface' | 'surfaces'>) {
	const values = (options.surfaces?.trim() || options.surface || 'integrated')
		.split(',')
		.map((entry) => entry.trim())
		.filter(Boolean);
	const selected = new Set<IntegratedDevCommandId>();
...
export function nodeLocalRuntime(label: string): LocalRuntimeSelection {
	return {
		requested: 'local',
		provider: 'local',
		selected: 'node-local',
...
export function dockerComposeIsAvailable(env: NodeJS.ProcessEnv) {
	const docker = resolveToolBinary('docker', { env });
...
export function resetActionForPath(
	id: IntegratedDevResetAction['id'],
	label: string,
	path: string,
): IntegratedDevResetAction {
	return {
		id,
		label,
		kind: 'path',
...
export function uniqueResetActions(actions: IntegratedDevResetAction[]) {
...
export function knownLocalRuntimeStateResetActions(tenantRoot: string, activePersistTo: string) {
	...
	].filter((action): action is IntegratedDevResetAction => Boolean(action));
...
export function isManagedMarketPostgresUrl(value: string | undefined) {
	...
	try {
		...
		return ['postgres:', 'postgresql:'].includes(url.protocol)
			...
			&& port === String(DEFAULT_MARKET_POSTGRES_PORT)
...
export function createIntegratedDevResetPlan(options: {
	tenantRoot: string;
	env: NodeJS.ProcessEnv;
	enabled?: boolean;
}): IntegratedDevResetPlan | null {
	if (!options.enabled) {
		return null;
	}
	...
	return {
		...
		actions: [
			...
			} satisfies IntegratedDevResetAction] : []),
