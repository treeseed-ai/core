import { NONCE_TTL_SECONDS, RATE_LIMIT_MAX_ATTEMPTS, RATE_LIMIT_TTL_SECONDS } from '../service/constants';
import { hashValue } from './crypto';
import type { KvNamespaceLike } from '../../../types/cloudflare';

type CounterRecord = { count: number; expiresAt: number };

const localNonceStore = (globalThis as { FixtureNonceStore?: Map<string, number> }).FixtureNonceStore
	?? new Map<string, number>();
const localRateStore = (globalThis as { FixtureRateStore?: Map<string, CounterRecord> }).FixtureRateStore
	?? new Map<string, CounterRecord>();

(globalThis as { FixtureNonceStore?: Map<string, number> }).FixtureNonceStore = localNonceStore;
(globalThis as { FixtureRateStore?: Map<string, CounterRecord> }).FixtureRateStore = localRateStore;
