...
import { assertNonceUnused, assertNonceUnusedLocal, applySubmissionRateLimit, applySubmissionRateLimitLocal } from '../../security/guard';
import { createContactSubmission } from '../../persistence/contact-submissions';
import { upsertSubscriber } from '../../persistence/subscribers';
import { sendEmail as sendDefaultEmail } from '../../transport/smtp';
import { verifyTurnstileToken as verifyDefaultTurnstileToken } from '../../security/turnstile';
...
function createDefaultSubscriberStore(runtime: FormRuntimeCapabilities, db: D1DatabaseLike | null): SubscriberStore {
	return {
		async upsert(input) {
			...
			const { upsertLocalSubscriber } = await import('../../runtime/subscribers-local');
...
function createDefaultContactStore(runtime: FormRuntimeCapabilities, db: D1DatabaseLike | null): ContactSubmissionStore {
	return {
		async create(input) {
			...
			const { createLocalContactSubmission } = await import('../../runtime/contact-submissions-local');
			return createLocalContactSubmission(input);
		},
	};
}

export const BUILTIN_FORMS_PROVIDERS: Record<string, FormsProvider> = {
	store_only: {
		id: 'store_only',
		behavior: {
...
export function finalizeFormsProvider(provider: FormsProvider): Required<FormsProvider> {
	return {
		...provider,
