...
export type WebRequestClassification =
	| { kind: 'static_asset' }
...
function configuredSourcePagePaths() {
	const configured = getDeployConfig().surfaces?.web?.cache?.sourcePages?.paths;
...
export function classifyWebRequest(request: Request, url: URL): WebRequestClassification {
...
function resolveWebCachePolicy() {
	const cache = getDeployConfig().surfaces?.web?.cache ?? {};
	return {
		sourcePages: {
...
export function applyWebCacheHeaders(
	request: Request,
	url: URL,
	response: Response,
) {
	const classification = classifyWebRequest(request, url);
